			...
			<div class="container">
				...
				<div class="row">
					...
					<div class="col-md-8 tyche-has-sidebar">
							<h4>디자이너 목록</h4>
					<form action='designBoardList.do' method='post'>
					</form>	
					<table border="1">
					<tr>
						...
						<th bgcolor="gray" width="150">핸드폰번호</th>
									...
									<!-- DB값 호출 -->			
						<c:forEach items="${designList}" var="designer">
							<tr>
								<td><a href="designSaleList.do?designerId=${designer.userId}">${designer.userId}</a></td><!--위에 VAR명이랑 일치  -->
								<td>${designer.userPwd}</td>
								<td>${designer.userName}</td>
								<td>${designer.userEmail}</td>
								<td>${designer.userBirth}</td>
								<td>${designer.userPhone}</td>
							</tr>
						</c:forEach>
					</table>
				</div>
