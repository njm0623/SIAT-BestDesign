				...
				<div class="row">
					...
					<tr>
						...
						<th bgcolor="gray" width="150">작품가격</th>			
									...
									<!-- DB값 호출 -->			
						<c:forEach items="${registWrite}" var="regi">
							<tr>
					...
					<table border="1">						
					<tr>
						<th bgcolor="gray" width="100">리뷰번호</th>
						<th bgcolor="gray" width="100">판매글번호</th>
						<th bgcolor="gray" width="100">등록 아이디</th>
						<th bgcolor="gray" width="150">등록 날짜</th>
						<th bgcolor="gray" width="100">평점</th>
						<th bgcolor="gray" width="150">내용</th>			
					</tr>
									<!-- DB값 호출 -->			
						<c:forEach items="${registReview}" var="rev">
							<tr>
								<td>${rev.reviewNum}</td><!--위에 VAR명이랑 일치  -->
								<td>${rev.saleNum}</td>
								<td>${rev.userId}</td>
								<td>${rev.reviewDate}</td>
								<td>${rev.reviewRate}</td>
								<td>${rev.reviewComent}</td>
							</tr>
						</c:forEach>
					</table>
				</div>
